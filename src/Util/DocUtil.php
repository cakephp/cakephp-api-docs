...
class DocUtil
{
    ...
    /**
     * @param string|null $comment Full docblock comment
     * @return \PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode
     */
    public static function parseBlock(?string $comment): PhpDocNode
    {
        static::init();

        if ($comment) {
            return static::$docParser->parse(new TokenIterator(static::$docLexer->tokenize($comment)));
        }
