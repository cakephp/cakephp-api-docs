...
class Factory
{
    ...
    public function createDefine(Const_ $node, Context $context): ReflectedDefine
    {
        ...
        $const = new ReflectedDefine($context->qualifyName($node->name->name), $doc, $context, $source);
    ...
    public function createFunction(Function_ $node, Context $context): ReflectedFunction
    {
        ...
        $func = new ReflectedFunction($context->qualifyName($node->name->name), $doc, $context, $source);
    ...
    public function createInterface(Interface_ $node, Context $context): ReflectedInterface
    {
        ...
        $interface = new ReflectedInterface($context->qualifyName($node->name->name), $doc, $context, $source);
    ...
    public function createClass(Class_ $node, Context $context): ReflectedClass
    {
        ...
        $class = new ReflectedClass($context->qualifyName($node->name->name), $doc, $context, $source);
    ...
    public function createTrait(Trait_ $node, Context $context): ReflectedTrait
    {
        ...
        $trait = new ReflectedTrait($context->qualifyName($node->name->name), $doc, $context, $source);
    ...
    protected function createConstant(
        ...
        $const = new ReflectedConstant(
            $classLike->qualifiedName() . '::' . $constNode->name->name,
            $doc,
            $classLike->context,
            $source
        );
    ...
    protected function createProperty(
        ...
        $prop = new ReflectedProperty(
            $classLike->qualifiedName() . '::' . $propNode->name->name,
            $doc,
            $classLike->context,
            $source
        );
        ...
        if (isset($doc->tags['var'])) {
            $prop->type = $doc->tags['var']->type;
        } else {
            $prop->type = $classNode->type ? DocUtil::parseType(PrintUtil::node($classNode->type)) : null;
        }
    ...
    protected function createMethod(ReflectedClassLike $classLike, ClassMethod $node): ReflectedMethod
    {
        ...
        $func = new ReflectedMethod(
            $classLike->qualifiedName() . '::' . $node->name->name,
            $doc,
            $classLike->context,
            $source
        );
    ...
    protected function createParam(ReflectedFunction $func, Param $node): ReflectedParam
    {
        ...
        if ($tag) {
            ...
            $param->type = $node->type ? DocUtil::parseType(PrintUtil::node($node->type)) : null;
        }
    ...
    protected function reflectFuncLike(ReflectedFunction $func, FunctionLike $node, DocBlock $doc): void
    {
        ...
        if (isset($doc->tags['return'])) {
            $func->returnType = $doc->tags['return']->type;
        } elseif ($node->getReturnType() !== null) {
            $func->returnType = DocUtil::parseType(PrintUtil::node($node->getReturnType()));
        }
    }
